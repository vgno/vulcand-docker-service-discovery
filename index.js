...
const IP = (interfaces[process.env.INTERFACE || 'eth0'] || []).find(i => i.family === 'IPv4').address;
...
const server = http.createServer((req, res) => co(function *() {
  ...
  yield Promise.each(containers, containerInfo => co(function *() {
    ...
    if (vhost) {
      ...
      if (!query.vhost || query.vhost === vhost) {
